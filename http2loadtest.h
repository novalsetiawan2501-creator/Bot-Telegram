...
#define DEFAULT_MAX_CONCURRENT_STREAMS  200
#define DEFAULT_DURATION_SEC            30
#define DEFAULT_CONNECTIONS             300
...
typedef struct {
    ...
    /* proxy stuff - NO proxy_workers, NO proxy_offset */
    bool   use_proxy;
    proxy_t *proxies;
    int    proxy_count;
} config_t;
...
typedef struct connection {
    ...
    int proxy_idx;               /* index proxy yang dipake buat koneksi ini */
} connection_t;
...
typedef struct worker {
    ...
    /* NO proxy_offset - using global atomic counter instead */
} worker_t;
...
/* ---------------------------------------------------------------------- */
/* global atomic counter for proxy rotation (defined in connection.c)     */
/* ---------------------------------------------------------------------- */
extern _Atomic uint64_t g_proxy_round_robin;
...
/* ---------------------------------------------------------------------- */
/* proxy.c - fungsi buat proxy handling                                   */
/* ---------------------------------------------------------------------- */
