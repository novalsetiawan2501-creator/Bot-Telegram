...
#define DEFAULT_PROXY_WORKERS           4
...
typedef struct {
    ...
    proxy_t *proxies;
    int    proxy_count;
    int    proxy_workers;        /* -w parameter: jumlah worker buat rotasi proxy */
} config_t;
...
typedef struct worker {
    ...
    int proxy_offset;            /* offset buat rotasi proxy tiap worker */
} worker_t;
...
/* ---------------------------------------------------------------------- */
/* proxy.c - fungsi baru buat proxy handling                              */
/* ---------------------------------------------------------------------- */
bool proxy_connect(connection_t *c, proxy_t *proxy);
void proxy_rotate(worker_t *w, connection_t *c);
