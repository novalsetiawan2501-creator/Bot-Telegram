...
static void print_usage(const char *prog) {
    printf(
        ...
        "  -w, --proxy-workers <n>    number of proxies per worker for rotation (default: %d)\n"
        ...
        "  * proxy mode: all connections will go through HTTP CONNECT proxy tunnel.\n"
        "  * Linux/epoll only.\n",
        prog, DEFAULT_DURATION_SEC, DEFAULT_CONNECTIONS, DEFAULT_MAX_CONCURRENT_STREAMS,
        DEFAULT_WARMUP_TIMEOUT_SEC, DEFAULT_DRAIN_TIMEOUT_SEC, DEFAULT_PROXY_WORKERS);
}
...
int main(int argc, char **argv) {
    ...
    config.proxies = NULL;
    config.proxy_count = 0;
    config.proxy_workers = DEFAULT_PROXY_WORKERS;
    ...
    static struct option long_opts[] = {
        ...
        {"proxy-workers", required_argument, 0, 'w'},
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "u:d:r:t:c:m:W:D:p:w:kvh", long_opts, NULL)) != -1) {
        switch (opt) {
        ...
        case 'w': config.proxy_workers = atoi(optarg); if (config.proxy_workers <= 0) config.proxy_workers = 1; break;
    ...
    if (config.use_proxy) {
        LOG_INFO("proxy:       enabled (%d proxies, %d per worker)", 
                 config.proxy_count, config.proxy_workers);
    }
    LOG_INFO(" ");
    ...
    for (int i = 0; i < config.num_threads; i++) {
        ...
        workers[i].proxy_offset = i * config.proxy_workers; /* unique proxy rotation per worker */
    }
