...
static void handle_timer_tick(worker_t *w) {
    ...
    if (w->config->rate <= 0.0) {
        /* 🔥 TURBO MODE: BUSY LOOP SAMPE SEMUA KONEKSI PENUH 🔥 */
        while (!w->draining && !atomic_load_explicit(&g_stop, memory_order_relaxed)) {
            bool any_issued = false;
            
            /* Loop semua koneksi, gas terus sampe ga bisa issue lagi */
            ...
            /* Kalo ga ada yang bisa diissue, berarti semua koneksi udah full 
               Kasih jeda 100 microsecond biar ga 100% CPU terlalu gila */
            if (!any_issued) {
                usleep(100); /* 0.10ms pause - cukup buat ngurangin CPU spike */
            }
            
            /* Kalo masih ada yang bisa diissue, loop langsung lagi tanpa nunggu */
        }
        return;
    }
