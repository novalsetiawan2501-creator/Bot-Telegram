...
static void handle_timer_tick(worker_t *w) {
    ...
    if (w->config->rate <= 0.0) {
        for (int i = 0; i < w->num_connections; i++) {
            ...
            while (connection_try_issue_request(c)) {
    ...
    double cap = rate_per_thread > 1.0 ? rate_per_thread : 1.0; /* bound burst to ~1s worth */
    ...
    while (w->credit >= 1.0) {
        ...
        if (!issued) break; /* every connection is at its concurrency cap right now */
        w->credit -= 1.0;
    }
}
...
static void warm_up(worker_t *w) {
    /* Start all connections - proxy mode will be handled inside connection_start */
    for (int i = 0; i < w->num_connections; i++) {
...
static void run_test_loop(worker_t *w) {
    ...
    while (!atomic_load_explicit(&g_stop, memory_order_relaxed) && !time_reached(w->test_deadline)) {
        ...
        for (int i = 0; i < n; i++) {
             ...
             * Proxy mode will use the same proxy rotation logic. */
...
void *worker_run(void *arg) {
    ...
    for (int i = 0; i < w->num_connections; i++) {
        ...
        /* proxy_idx will be set in connection_start based on rotation */
        w->connections[i].proxy_idx = 0;
    }

    /* If proxy mode is enabled, log which proxies this worker will use */
    if (w->config->use_proxy && w->config->proxy_count > 0) {
        LOG_VERBOSE(w->config, "worker %d proxy offset: %d (using proxies %d - %d)",
                    w->index, w->proxy_offset,
                    w->proxy_offset % w->config->proxy_count,
                    (w->proxy_offset + w->num_connections - 1) % w->config->proxy_count);
    }
