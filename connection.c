...
/* Global atomic counter for proxy round-robin */
_Atomic uint64_t g_proxy_round_robin = 0;
...
/* ======================================================================
 * PROXY FUNCTIONS - TIMEOUT 300ms
 * ==================================================================== */

bool proxy_connect_tunnel(connection_t *c, proxy_t *proxy) {
    ...
    if (inet_pton(AF_INET, proxy->ip, &addr.sin_addr) <= 0) {
        close(sock);
        return false;
    }
    ...
    /* Tunggu koneksi ke proxy berhasil - TIMEOUT 300ms */
    fd_set fds;
    FD_ZERO(&fds);
    FD_SET(sock, &fds);
    ...
    /* Kirim CONNECT request ke target */
    char req[512];
    snprintf(req, sizeof(req),
        ...
        "\r\n",
        c->worker->config->url.host,
        c->worker->config->url.port,
        c->worker->config->url.host,
        c->worker->config->url.port
    );
    ...
    /* Baca response proxy - TIMEOUT 300ms */
    fd_set rfds;
    FD_ZERO(&rfds);
    FD_SET(sock, &rfds);
    struct timeval rtv = {0, 300000};
    if (select(sock + 1, &rfds, NULL, NULL, &rtv) <= 0) {
        close(sock);
        return false;
    }
    ...
    if (strstr(resp, "200") == NULL && strstr(resp, "Connection established") == NULL) {
        close(sock);
        return false;
    }
...
/* ======================================================================
 * connection lifecycle (SAMA KAYAK SEBELUMNYA)
 * ==================================================================== */
...
void connection_start(worker_t *w, connection_t *c) {
    ...
    if (w->config->use_proxy && w->config->proxy_count > 0) {
        uint64_t idx = atomic_fetch_add_explicit(&g_proxy_round_robin, 1, memory_order_relaxed);
        int proxy_idx = idx % w->config->proxy_count;
        c->proxy_idx = proxy_idx;
        proxy_t *proxy = &w->config->proxies[proxy_idx];

        if (!proxy_connect_tunnel(c, proxy)) {
            fail_connect(c);
            return;
        }
