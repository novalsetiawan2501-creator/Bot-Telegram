 ...
 *
 * These callbacks read/write directly into the buffers nghttp2 gives us,
 * so there is no extra buffering on our side. Each connection's session
 * is only ever touched by the worker thread that owns it, so none of
 * this needs locks.
 * ==================================================================== */
...
/* 🔥🔥🔥 TAMBAHAN PERTAMA: CALLBACK UNTUK FORCE OVERRIDE SETTINGS DARI SERVER 🔥🔥🔥 */
static int on_frame_recv_cb(nghttp2_session *session, const nghttp2_frame *frame, void *user_data) {
    connection_t *c = user_data;
    if (frame->hd.type == NGHTTP2_SETTINGS) {
        /* SERVER MAU LIMIT? IGNORE! */
        nghttp2_session_set_remote_settings(
            session, 
            NGHTTP2_SETTINGS_MAX_CONCURRENT_STREAMS, 
            (uint32_t)c->worker->config->max_concurrent_streams
        );
    }
    return 0;
}
...
void connection_update_epoll(connection_t *c) {
    ...
    if (c->state == CONN_CONNECTING) {
        ...
        events = EPOLLIN | EPOLLOUT; /* handshake direction can flip either way */
    } else if (c->session) {
...
static bool init_h2_session(connection_t *c) {
    ...
    /* 🔥🔥🔥 TAMBAHAN KEDUA: REGISTRASI CALLBACK 🔥🔥🔥 */
...
static void begin_tls_or_h2(connection_t *c) {
    if (c->worker->config->url.tls) {
        ...
        connection_handle_event(c, 0); /* drive the handshake forward right away */
        return;
    }
...
bool connection_try_issue_request(connection_t *c) {
    ...
    if (c->active_streams >= c->worker->config->max_concurrent_streams) return false;
...
void connection_close(connection_t *c) {
    ...
    if (c->session) {
        /* Best-effort final GOAWAY so the server knows we are done. */
