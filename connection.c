...
static bool proxy_connect_tunnel(connection_t *c, proxy_t *proxy) {
    ...
    /* Tunggu koneksi ke proxy berhasil (non-blocking poll) */
    fd_set fds;
    FD_ZERO(&fds);
    FD_SET(sock, &fds);
    ...
    /* Cek apakah proxy ngasih 200 Connection established */
...
void connection_start(worker_t *w, connection_t *c) {
    ...
    /* ============================================================
     * PROXY MODE: konek lewat proxy
     * ============================================================ */
    if (w->config->use_proxy && w->config->proxy_count > 0) {
        /* Pilih proxy berdasarkan round-robin dengan offset worker */
        int idx = (c->index + w->proxy_offset) % w->config->proxy_count;
        c->proxy_idx = idx;
        proxy_t *proxy = &w->config->proxies[idx];

        if (!proxy_connect_tunnel(c, proxy)) {
            LOG_VERBOSE(w->config, "proxy connect failed: %s:%d", proxy->ip, proxy->port);
            fail_connect(c);
            return;
        }

        /* Set non-blocking setelah tunnel established */
    ...
    /* ============================================================
     * DIRECT MODE: konek langsung ke target
     * ============================================================ */
